...
cleanup() {
    ...
    yum list installed | grep remi | cut -d\  -f1 | sed -e '/^$/d' | xargs yum remove -y
    ...
    rm -fr /var/www/fusionpbx /var/cache/fusionpbx /etc/fusionpbx
    rm -fr /usr/share/nginx /var/cache/nginx /etc/nginx /var/log/nginx
    rm -fr /var/lib/php /usr/lib64/php /var/log/php-fpm /etc/php*
    ...
    rm -fr /var/lib/fail2ban /etc/fail2ban
    rm -fr /usr/src/fusionpbx-install.sh/ /etc/selinux/targeted/active/modules/100/{memcached,fail2ban}
    rm -fr /var/lib/pgsql

    for i in fusionpbx nginx memcached php postgres freeswitch fail2ban; do find / -name $i -print; done
...
"
    ...
    mv /etc/nginx/nginx.tmp /etc/nginx/nginx.conf && rm -f /etc/nginx/nginx.tmp
    sed -i /etc/nginx/nginx.conf -e '/include \/etc\/nginx\/conf\.d\/\*\.conf\;/a \    include \/etc\/nginx\/sites-enabled\/\*\.conf\;'
    sed -i /etc/nginx/nginx.conf -e "s/^\(user\)[[:blank:]]*\(nginx\);$/user freeswitch daemon;/g"
    sed -i /etc/nginx/nginx.conf -e "s|^\(pid\)[[:blank:]]*\(.*\);$|pid /var/run/nginx.pid;|g" /etc/nginx/nginx.conf
...
phpini() {
    ...
    yum --enablerepo=remi,remi-php72 install -y php-fpm php-pgsql php-curl php-opcache php-common php-pdo php-soap libargon2 php-odbc php-imap php-xml php-xmlrpc php-cli php-gd php-snmp php-ldap

    if [ ! -d /etc/php-fpm.d.bkp ]; then
        cp /etc/php.ini /etc/php.ini.bkp
        cp -av /etc/php-fpm.d /etc/php-fpm.d.bkp
	cp /etc/php-fpm.conf /etc/php-fpm.conf.bkp
    fi

    sed -i /etc/php.ini -e "s|^[[:blank:]]*;*[[:blank:]]*\(date.timezone\)[[:blank:]]*=[[:blank:]]*.*|date.timezone = $TZONE|g"
    sed -i /etc/php.ini -e "s|^[[:blank:]]*;*[[:blank:]]*\(cgi.fix_pathinfo\)[[:blank:]]*=[[:blank:]]*.*|cgi.fix_pathinfo = 0|g"
    sed -i /etc/php.ini -e "s|^[[:blank:]]*;*[[:blank:]]*\(post_max_size\)[[:blank:]]*=[[:blank:]]*.*|post_max_size = 80M|g"
    sed -i /etc/php.ini -e "s|^[[:blank:]]*;*[[:blank:]]*\(upload_max_filesize\)[[:blank:]]*=[[:blank:]]*.*|upload_max_filesize = 80M|g"
    sed -i /etc/php.ini -e "s|^[[:blank:]]*;*[[:blank:]]*\(max_input_vars\)[[:blank:]]*=[[:blank:]]*.*|max_input_vars = 8000|g"
    diff -y /etc/php.ini /etc/php.ini.bkp | grep ' |' | grep '='

    sed -i /etc/php-fpm.d/www.conf -e "s|^;*[[:blank:]]*\(user\)[[:blank:]]*=.*|user = freeswitch|g"
    sed -i /etc/php-fpm.d/www.conf -e "s|^;*[[:blank:]]*\(group\)[[:blank:]]*=.*|group = daemon|g"
    sed -i /etc/php-fpm.d/www.conf -e "s|^;*[[:blank:]]*\(listen\)[[:blank:]]*=.*|listen = /var/run/php-fpm/php-fpm.sock|g"
    sed -i /etc/php-fpm.d/www.conf -e "s|^;*[[:blank:]]*\(listen\.owner\)[[:blank:]]*=.*|listen\.owner = freeswitch|g"
    sed -i /etc/php-fpm.d/www.conf -e "s|^;*[[:blank:]]*\(listen\.group\)[[:blank:]]*=.*|listen\.group = daemon|g"
    sed -i /etc/php-fpm.d/www.conf -e "s|^;*[[:blank:]]*\(listen.acl_users\)[[:blank:]]*=.*|;listen.acl_users = freeswitch,daemon|g"
    diff -y /etc/php-fpm.d/www.conf /etc/php-fpm.d.bkp/www.conf | grep ' |' | grep '='

    sed -i /etc/php-fpm.conf -e "s#^.*pid.*#pid = /var/run/php-fpm/php-fpm.pid#g"
    diff -y /etc/php-fpm.conf /etc/php-fpm.conf.bkp | grep ' |' | grep '='
...
EOF
    ...
    cp /usr/src/fusionpbx-install.sh/centos/resources/fusionpbx/config.php /etc/fusionpbx
    ...
    cd /var/www/fusionpbx
    php /var/www/fusionpbx/core/upgrade/upgrade_domains.php
    ls –l /usr/share/freeswitch/scripts
...
versions() {
    ...
    ls -la /var/run/{nginx,php-fpm}*
    ls -la /var/log/{nginx,php-fpm}*
    ps -o uid,uname:10,gid,group:10,pid,ppid,args -afx | egrep "(USER|postgres|nginx|php|freeswitch|fail2ban)" | grep -v grep
}
